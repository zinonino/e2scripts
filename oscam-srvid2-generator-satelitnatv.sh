...
###         and then generate the 'oscam.srvid2' file from parsed data         ###
#################################################################################
...
create_srvid_file() 
{
    ...
    #    $2 = provider name (that exact name will be inserted into the '.srvid2' output file)
    ...
    FILE_OUTPUT="/tmp/oscam__${FILE_NAME}.srvid2"
    rm -f $FILE_OUTPUT
    ...
    while IFS= read -r LINE; do
        ...
        echo "${SIDHEX}:${3}|${SRN}|||${2}" >> $FILE_OUTPUT
    done <<< "$LIST"
...
create_srvid_file "https://www.satelitnatv.sk/frekvencie/antik-sat-sk/" "AntikSat 16E" "0B00"

#create_srvid_file "https://www.satelitnatv.sk/skylink-programy-frekvencie-parametre/" "Skylink" "0D96,0624"
#create_srvid_file "https://www.satelitnatv.sk/antik-sat/" "AntikSat 16E" "0B00"
#create_srvid_file "https://www.satelitnatv.sk/freesat-by-upc-direct/" "FreeSAT" "0D97,0653,0B02"



### backup the original file "oscam.srvid2" to the "/tmp" dir:
...
rm -f /tmp/oscam__*
[ -f "$OSCAM_SRVID" ] && echo "All generated '.srvid2' files have been merged into one and moved to the directory:  ${OSCAM_SRVID}"
